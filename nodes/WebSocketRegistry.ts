...
export class WebSocketRegistry {
  ...
  private createServer(serverId: string, config: IServerConfig) {
    ...
    const pingInterval = setInterval(() => {
      ...
      console.error(
        `[DEBUG-REGISTRY] Sending ping to ${clients.size} clients on server ${serverId}`
      )
      clients.forEach((client, clientId) => {
        if (client.readyState === WebSocket.OPEN) {
          try {
            ...
            console.error(
              `[DEBUG-REGISTRY] Error sending ping to client ${clientId}:`,
              error
            )
          }
        } else if (
          ...
          console.error(
            `[DEBUG-REGISTRY] Removing dead client ${clientId} from server ${serverId}`
          )
    ...
    wss.on("connection", (ws: WebSocket) => {
      ...
      console.error(
        `[DEBUG-REGISTRY] New client connected. Server ID: ${serverId}, Client ID: ${clientId}`
      )
      ...
      ws.on("message", (message: WebSocket.Data) => {
        console.error(
          `[DEBUG-REGISTRY] Received message from client ${clientId} on server ${serverId}`
        )
        try {
      ...
      ws.on("close", () => {
        clients.delete(clientId)
        console.error(
          `[DEBUG-REGISTRY] Client ${clientId} disconnected from server ${serverId}`
        )
        console.error(`[DEBUG-REGISTRY] Active Clients: ${clients.size}`)
        this.listClients(serverId)
        this.saveRegistry()
      ...
      ws.on("error", error => {
        console.error(
          `[DEBUG-REGISTRY] WebSocket error for client ${clientId}:`,
          error
        )
      })
    })
    ...
    wss.close = function (...args) {
      ...
      console.error(
        `[DEBUG-REGISTRY] Stopping ping interval for server ${serverId}`
      )
    ...
    if (server) {
      ...
      if (address && typeof address === "object") {
        ...
        if (currentPort !== config.port || currentPath !== config.path) {
          console.error(
            `[DEBUG-REGISTRY] Server config changed for ${serverId}. Port: ${currentPort} -> ${config.port}, Path: ${currentPath} -> ${config.path}`
          )
  ...
  public async closeServer(
    serverId: string,
    options: { keepClientsAlive?: boolean; executionId?: string } = {}
  ): Promise<void> {
    const keepClientsAlive = options.keepClientsAlive !== false // Default to true unless explicitly set to false
    const executionId = options.executionId

    console.error(
      `[DEBUG-REGISTRY] Attempting to close server with ID: ${serverId}. Keep clients alive: ${keepClientsAlive}, Execution ID: ${
        executionId || "none"
      }`
    )
    const server = this.servers.get(serverId)
    ...
    if (server) {
      ...
      if (executionId && server.activeExecutions) {
        ...
        console.error(
          `[DEBUG-REGISTRY] Registered execution ${executionId} for server ${serverId}. Active executions: ${server.activeExecutions.size}`
        )
      }
      ...
      if (executionId && server.activeExecutions && !keepClientsAlive) {
        ...
        console.error(
          `[DEBUG-REGISTRY] Removed execution ${executionId} from server ${serverId}. Remaining executions: ${server.activeExecutions.size}`
        )
      }

      // If there are active executions, always keep clients alive regardless of the passed parameter
      const hasActiveExecutions =
        server.activeExecutions !== undefined &&
        server.activeExecutions.size > 0
      const shouldKeepAlive = keepClientsAlive || hasActiveExecutions

      if (hasActiveExecutions) {
        console.error(
          `[DEBUG-REGISTRY] Server ${serverId} has ${server.activeExecutions?.size} active executions - forcing keepClientsAlive to true`
        )
      }
      ...
      if (!shouldKeepAlive) {
        ...
        server.clients.forEach(client => {
          try {
            ...
            console.error(
              `[DEBUG-REGISTRY] Error closing client connection:`,
              error
            )
          }
        })
        ...
        console.error(
          `[DEBUG-REGISTRY] Keeping ${server.clients.size} clients alive for server ${serverId}`
        )
      }
      ...
      if (!shouldKeepAlive) {
        ...
        await new Promise<void>(resolve => {
          server.wss.close(() => {
            console.error(
              `[DEBUG-REGISTRY] Server fully closed. ID: ${serverId}`
            )
            resolve()
          })
        })
        ...
        console.error(
          `[DEBUG-REGISTRY] Server closed successfully. ID: ${serverId}`
        )
      } else {
        // For soft close, we keep the server entry and all connections
        console.error(
          `[DEBUG-REGISTRY] Server soft-closed, connections maintained for ${serverId}`
        )
      }
  ...
  public broadcastToServer(
    ...
    if (!server) {
      console.error(
        `[DEBUG-REGISTRY] Server with ID ${serverId} not found for broadcast`
      )
      return
    }

    console.error(
      `[DEBUG-REGISTRY] Broadcasting message to server ${serverId} with ${server.clients.size} clients`
    )
    server.clients.forEach((client, clientId) => {
      try {
        if (client.readyState === WebSocket.OPEN) {
          ...
          console.error(
            `[DEBUG-REGISTRY] Skipping client ${clientId} - not open (state: ${client.readyState})`
          )
        }
      } catch (error) {
        console.error(
          `[DEBUG-REGISTRY] Error sending message to client ${clientId}:`,
          error
        )
      }
    })
  ...
  public unregisterExecution(serverId: string, executionId: string): void {
    ...
    if (server && server.activeExecutions) {
      server.activeExecutions.delete(executionId)
      console.error(
        `[DEBUG-REGISTRY] Unregistered execution ${executionId} from server ${serverId}. Remaining executions: ${server.activeExecutions.size}`
      )
    }
  }
}
